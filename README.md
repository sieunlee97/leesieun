...
- 스프링 테스트 pom.xml(외부라이브러리 가져다가 사용하는 방식) 디펜던시 의존성 추가OK.
- Maven 기반 전자정부표준프레임워크 egov3.9버전 -> 3.10으로 업드레이드OK.
...
- junit으로 DB접근 후 관리자단 회원관리 CRUD unit테스트 마무리.OK
...
- > IoC(Inversion of Controller: 개발자가 하던 메모리관리(가비지컬렉션-쓰레기프로그램종료시키기), 프로그램 생성추가관리를 개발자가 코딩으로 하지 않고, 스프링이 대신 처리하는 기능
