#### 20201112(목)
- 실습4 : 헤로쿠 클라우드에 leesieun 프로젝트 배포, 도메인URL로 크롬에서 확인
...
- 깃에 올릴 때, 제외시킬 폴더 지정. target폴더는 깃에서 제외시키는 방법
- .gitignore 파일을 만들어서, target 이름을 적음
- root : 최상위 위치를 가리킨다.
- POM.xml 파일에서 버전 변경
- Program 에 사용되는 Object Module(외부 프로그램들 = 모듈) 관리하는 파일을 pom.xml
...
- 이클립스 File 메뉴> 제일 하단 바로 위 Restart 클릭
- 자바버전 : 1.6 -> 1.8 변경
