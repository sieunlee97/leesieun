...
## v16.0.0 네트워크 애플리케이션 구현
### 20210416(금)
- <실습>
- 오픈 차트의 Json 데이터를 DB에서 CRUD 처리하기 
- 스프링 투표용 RestAPI 서버 생성 (JsonDataController 클래스) 

### 20210415(목)
- <실습>
- 오픈차트 Json데이터 UI처리 작업
- 스프링 프로젝트에 사용된 오픈소스 (아래)
- **AdminLTE(부트스트랩UI템플릿), 서머노트(웹에디터), 마리아DB(오픈소스), 오픈차트(대시보드)**

### 20210414(수)
- <실습>
- 스프링 RestAPI와 안드로이드앱 비동기 통신 진행

## v15.0.0 임베디드 애플리케이션 구현
### 20210413(화)
- <이론>
- 스프링 프레임워크 특징 : **DI, IoC, AOP**
- IoC(제어의역전) : 스프링 전용. 개발자가 관리하는 객체 라이프 사이클(생명주기)을 스프링이 관리
- 라이프사이클 : 생성 - 대기 - 실행 - 소멸 / 개발자가 처리
- IaaS : Infra as A Service = AWS,GCP = 대지 
- PaaS : Platform as A Service = PaasTA, Heroku = 건물
- SaaS : Software as A Serivce = 스프링프로젝트, egov프로젝트 = 방
...
- > 사용자 그룹과 관리자 그룹의 메뉴 달라짐 
- > 사용자그룹 : 마이페이지 / 관리자 그룹 : AdminLTE
